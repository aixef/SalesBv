({
    ...
    serverCall : function(component, event, helper) {
        ...
        if (event.keyCode === 27 || !searchText.trim()) {
            ...
            var field_API_noCl = component.get("v.field_API_noCliente");
            var field_API_search = component.get("v.field_API_search");
            var field_API_ugUsu = component.get("v.field_API_ugUsuario");
            var field_API_bancaUsu = component.get("v.field_API_bancaUsuario");
            var field_API_divUsu = component.get("v.field_API_divisionUsuario");
            var field_API_divPropCl = component.get("v.field_API_divisionPropCuenta");
            ...
            action.setParams({
                ...
                fldAPINoCl : field_API_noCl,
                fieldAPIUg : field_API_ugUsu,
                fieldAPIDiv : field_API_divUsu,
                fieldAPIBanca : field_API_bancaUsu,
                fieldAPIDivCl:field_API_divPropCl,
                lim : limit,
    ...
    handleResponse : function (res,component,helper){
        if (res.getState() === 'SUCCESS') {
            ...
            if(retObj.length <= 0){
                var noResult = JSON.parse('[{"text":"No se encontraron resultados"}]');
